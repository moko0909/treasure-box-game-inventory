  ...
  return (
    <div ref={containerRef} className="relative w-full h-full overflow-hidden bg-[#0A0A0A]">
      ...
      <div
        ...
        className="absolute left-0 right-0 flex flex-col rounded-t-[24px] border-t border-[#2C2C2C]"
        style={{
          top: sheetTop,
          bottom: 0,
          background: '#141414',
          boxShadow: '0 -8px 40px rgba(98,0,238,0.18), 0 -2px 12px rgba(0,0,0,0.8)',
      ...
      >
        ...
        <div className="flex-shrink-0 cursor-grab active:cursor-grabbing select-none">
          ...
          <div className="px-5 pt-2 pb-3">
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <h1 className="text-[17px] font-extrabold text-white tracking-tight leading-none">보물상자</h1>
                <p className="text-[11px] text-[#6A6A6A] mt-1">내 주변 게임샵 {stores.length}곳</p>
              </div>
              {isExpanded ? (
                <button
                  type="button"
                  onClick={() => goSnap('half')}
                  className="flex items-center gap-1.5 bg-[#1E1E1E] border border-[#2C2C2C] rounded-full px-3 py-1.5 text-[12px] font-bold text-[#BB86FC] active:scale-95 transition-transform"
                >
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#BB86FC" strokeWidth="2.5" aria-hidden="true">
                    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" />
                  </svg>
                  지도
                </button>
              ) : (
                <div className="flex items-center gap-1.5 bg-[#6200EE]/15 text-[#BB86FC] rounded-full px-3 py-1.5 border border-[#6200EE]/30">
            ...
            <div className="relative mb-2.5">
              <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#4A4A4A]" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
              ...
              <input
                ...
                aria-label="매장 검색"
                className="w-full h-11 pl-10 pr-4 rounded-2xl text-sm text-white placeholder-[#4A4A4A] outline-none border border-[#2C2C2C] focus:border-[#6200EE] transition-colors"
              ...
              <button
                ...
                className={`h-9 px-4 rounded-full text-sm font-semibold flex-shrink-0 transition-all ${
                  platforms.size === 0
                    ...
                    : 'bg-[#1E1E1E] text-[#6A6A6A] border border-[#2C2C2C]'
                }`}
                style={platforms.size === 0 ? { background: 'linear-gradient(135deg,#6200EE,#9C27B0)' } : {}}
              >
                전체
              </button>
              {PLATFORMS.map((p) => (
        ...
        <div data-scroll className="flex-1 overflow-y-auto overscroll-contain px-5 pb-6" style={{ touchAction: 'pan-y' }}>
          <div className="flex items-center justify-between py-3 sticky top-0 z-10" style={{ background: '#141414' }}>
            <p className="text-[13px] font-bold text-white">매장 {stores.length}곳</p>
            <span className="text-[12px] text-[#6A6A6A] font-medium">거리순</span>
          </div>

          {stores.length === 0 ? (
            <div className="text-center py-14 px-6">
              <div className="w-12 h-12 mx-auto rounded-full bg-[#1E1E1E] border border-[#2C2C2C] flex items-center justify-center mb-3">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#4A4A4A" strokeWidth="2" aria-hidden="true">
              ...
              {hasFilter ? (
                <>
                  <p className="text-[#6A6A6A] text-sm font-medium">검색 결과가 없습니다.</p>
                  <button
                    type="button"
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {popular.map(({ game, best }) => (
                  <button
                    ...
                    className="flex-shrink-0 w-[112px] rounded-[14px] border border-[#2C2C2C] overflow-hidden active:scale-95 transition-transform text-left"
                    style={{ background: '#1E1E1E' }}
                  >
                    ...
                    <div className="p-2">
                      <p className="text-[11px] font-bold text-white leading-tight line-clamp-2">{game.title}</p>
                      <p className="text-[10px] text-[#6A6A6A] mt-1">{best ? `${best.store.distance}km` : '재고 없음'}</p>
                    </div>
                  </button>
                ))}
              </div>
